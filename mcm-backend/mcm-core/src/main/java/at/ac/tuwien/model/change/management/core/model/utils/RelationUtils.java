...
public class RelationUtils {
    public static final int RELATION_CONNECTED_TOLERANCE = 10;
    ...
    public static Model processRelations(Model original) {
        ...
        for (Node relationNode : relations) {
            Relation newRelation = Relation.fromNode(relationNode, res.getZoomLevel());
            if (newRelation == null) {
                continue;
            }
            ...
            for (Node n : res.getNodes()) {
                // find target
                if (RelationUtils.isConnected(n, newRelation.getEndPoint(), RELATION_CONNECTED_TOLERANCE)) {
                    relTarget = n;
                    break;
                }
            }
